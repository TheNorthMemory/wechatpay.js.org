---
title: ETC授权签约状态变化通知(JSON)
description: 当用户在微信车主小程序暂停，删除当前ETC扣费服务或解约车主服务时，微信会把相关结果异步通知到服务商的回调地址（回调地址需要事先申请配置）。
---

# {{ $frontmatter.title }} {#post}

{{ $frontmatter.description }} [官方文档](https://pay.weixin.qq.com/docs/partner/apis/etc-authorization/notice/get-state-notice.html)
...
```json
{
  ...
  "event_type":"VEHICLE.USER_STATE_CHANGE",
  "resource" : {
 ...
 * @prop {'VEHICLE.USER_STATE_CHANGE'} event_type
 ...
 * @prop {'OPENED'|'PAUSE'|'DELETED'} bind_state
...
> - 如果在所有通知频率(4小时)后没有收到微信侧回调，商户应调用[通过商户etc绑定号查询签约状态](/openapi/v3/vehicle/etc/contracts/{contract_id})。
