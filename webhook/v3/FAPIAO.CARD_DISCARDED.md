---
title: 发票卡券作废通知(JSON)
description: 当有发票卡券作废后，微信后台会把相关确认结果和订单信息发送给商户，商户需要接收处理该消息，并返回应答。
---

# {{ $frontmatter.title }} {#post}

{{ $frontmatter.description }} [官方文档](https://pay.weixin.qq.com/docs/merchant/apis/fapiao/fapiao-card-template/invoice-card-cancel-notice.html) [官方文档](https://pay.weixin.qq.com/docs/partner/apis/fapiao/fapiao-card-template/invoice-card-cancel-notice.html)
...
2. 校园轻松付交易类型，订单已受理，但未完成扣款时不返回(**trade_type=AUTH**)字段 {#EDUSCHOOLPAY}
 ...
 * @prop {'FAPIAO.CARD_DISCARDED'} event_type
 * @prop {{algorithm: string, ciphertext: string, nonce: string, associated_data: string}} resource
 * @typedef PlainObject
 * @prop {string} mchid
 * @prop {string=} sub_mchid
 * @prop {string} fapiao_apply_id
 * @prop {{fapiao_id: string, fapiao_status: string, card_status: string}} fapiao_information
 */
