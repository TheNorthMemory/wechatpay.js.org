---
title: 隔夜归还保险订单状态变化通知(JSON)
description: 租用充电宝-隔夜归还保险订单领取状态变化后，微信会把保险订单状态变化信息通知给商户。
---

# {{ $frontmatter.title }} {#post}

{{ $frontmatter.description }} [官方文档](https://pay.weixin.qq.com/docs/merchant/apis/hire-power-bank-insurance/insurance-orders/status-change-notice.html)
...
```json
{
  ...
  "event_type":"HIRE_POWER_BANK.RECEIVE_INSURANCE",
  "resource" : {
 ...
 * @prop {'HIRE_POWER_BANK.RECEIVE_INSURANCE'} event_type
 ...
 * @prop {string} openid
 * @prop {number} max_claim_count
 * @prop {number} claimed_count
 * @prop {string} order_receive_time
 * @prop {'RECEIVING'|'RECEIVED'|'FAILED'} order_receive_state
 * @prop {string} order_begin_time
 * @prop {string} order_end_time
 */
