---
title: 用户管理会员卡事件通知(JSON)
description: 当会员卡批次或者用户的会员卡发生变化时，微信会把相关事件结果和用户信息发送给商户，商户需要接收处理，并按照文档规范返回应答。
---

# {{ $frontmatter.title }} {#post}

{{ $frontmatter.description }} [官方文档](https://pay.weixin.qq.com/wiki/doc/apiv3/Offline/apis/chapter5_9_32.shtml)
...
```json
{
  ...
  "event_type":"MEMBERCARD.USERCARD_MANAGE",
  "resource" : {
...
```json
{
  "event_type":"USER_DELETE_MEMBERCARD",
 ...
 * @prop {'MEMBERCARD.USERCARD_MANAGE'} event_type
 * @prop {{algorithm: string, ciphertext: string, nonce: string, associated_data: string}} resource
 *
 * @typedef PlainObject
 * @prop {'USER_VIEW_MEMBERCARD'|'USER_DELETE_MEMBERCARD'|'USER_MODIFY_INFORMATION'} event_type
 * @prop {string} card_id
 * @prop {string} code
 * @prop {string} event_time
 * @prop {string} openid
 * @prop {string} unionid
 */
